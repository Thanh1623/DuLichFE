...
function ModalViewMove(props) {
    ...
    useEffect(() => {
        if (!_.isEmpty(dataView)) {
            ...
            setType(dataView.type_vehicle);
            setMap(dataView.map);

            setImage(base64ToFile(`data:image/jpeg;base64,${dataView.moves_image_base64}`));


            setImageP(`data:image/jpeg;base64,${dataView.moves_image_base64}`);
            setDataImagePreview({
                url: `data:image/jpeg;base64,${dataView.moves_image_base64}`,
            })
    ...
    const handleSubmitCreateUser = async () => {
        ...
        if (validation === true) {
            // call api
            let data = await putMove(dataView.moves_id, title, address, contentHTML, contentMarkdown,
                type, image, map)
